...
def core_only(request):
    return render_to_response('fresh_start_mag/core_only.html', locals(), context_instance=RequestContext(request))
    
def core_right(request):
    return render_to_response('fresh_start_mag/core_right.html', locals(), context_instance=RequestContext(request))